...
/// with supplied parameters. Saves network after training.
void TrainNetwork(Network& network, std::string mnist_path, std::string save_dir_path, int epochs, float learning_rate, int batch_size) {
    // Load training and testing data.
    NetworkData training_data = IOTools::LoadMnistDataPair(
        mnist_path + "/train-images.idx3-ubyte",
        mnist_path + "/train-labels.idx1-ubyte"
    );
    NetworkData testing_data = IOTools::LoadMnistDataPair(
...
void ClassifyIDXImage(Network& network, std::string image_path) {
    // Turn 2d array of ints into a flat array of floats.
    xarray<uint8_t> raw_data = IOTools::ReadIDX(image_path);
...
int main(int argc, char** argv) {
    ...
    if (network_load_dir_path.size() != 0) {
        network = Network(network_load_dir_path);
    }
